...
---STRUCTURED---
请额外输出一份结构化 JSON（用于生成 PDF）。根据简历实际内容自由识别所有区块，不要遗漏任何信息。格式如下：
```json
{{
  "name": "姓名",
  "title": "目标职位名称",
  ...
  "sections": [
    {{
      "title": "教育背景",
      "type": "timeline",
      "items": [{{"period": "2014.09-2018.06", "title": "学校名", "subtitle": "专业 · 学位"}}]
    }},
    {{
      "title": "专业技能",
      "type": "tags",
    ...
    {{
      "title": "工作经历",
      "type": "timeline",
      "items": [{{"period": "2020.01-2024.04", "title": "公司名", "subtitle": "职位", "desc": "工作描述"}}]
    }},
    {{
      "title": "项目经验",
      "type": "detailed",
      "items": [{{"name": "项目名", "role": "角色", "tech": "技术栈", "desc": ["描述1", "描述2"]}}]
    }},
    {{
      "title": "证书与荣誉",
      "type": "list",
...
```
type 说明：timeline（带时间线）、detailed（项目详情）、tags（标签列表）、list（简单列表）、text（文本段落）。
请根据简历实际区块自由添加 section，不限数量。如有论文发表、获奖、志愿活动等也请提取。JSON 必须合法。"""
...
{{
  ...
  "sections": [
    {{"title": "教育背景", "type": "timeline", "items": [{{"period": "起止时间", "title": "学校", "subtitle": "专业·学位"}}]}},
    {{"title": "专业技能", "type": "tags", "items": ["技能1", "技能2"]}},
    {{"title": "工作经历", "type": "timeline", "items": [{{"period": "起止时间", "title": "公司", "subtitle": "职位", "desc": "描述"}}]}},
    {{"title": "项目经验", "type": "detailed", "items": [{{"name": "项目名", "role": "角色", "tech": "技术栈", "desc": ["描述1", "描述2"]}}]}},
    {{"title": "证书", "type": "list", "items": ["证书1"]}},
    {{"title": "自我评价", "type": "text", "content": "评价内容"}}
  ]
}}
```
type: timeline（带时间线）、detailed（项目详情）、tags（标签）、list（列表）、text（文本）。根据简历实际区块自由添加 section。"""
